...
def calculate_demographic_data(print_data=True):
    ...
    rich_percentage = round((df.loc[num_min_workers & (df["salary"] == ">50K"), "salary"]).count(
    ) / (df.loc[num_min_workers, "salary"]).count() * 100, 1)

    # What country has the highest percentage of people that earn >50K?
    earning_country_percentage = df[df["salary"] == ">50K"]["native-country"].value_counts(
    ) / df["native-country"].value_counts() * 100
    highest_earning_country = earning_country_percentage.idxmax()
    highest_earning_country_percentage = round(
        earning_country_percentage.max(), 1)
